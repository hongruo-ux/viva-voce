...
const megaMenuData = {
  ...
  Summer: {
    categories: [
      { title: "SUMMER ESSENTIALS", items: ["All Summer","Resort Wear","Swimwear","Cover-Ups","Beach Bags"] },
      { title: "BY DESTINATION", items: ["Tropical Getaway","European Summer","Beach Vacation","City Escape"] },
      { title: "SHOP BY STYLE", items: ["Bohemian","Minimalist","Luxe Resort","Sporty Chic"] },
    ],
    featured: { title: "Resort Ready" },
  },
  Clothing: {
    categories: [
      { title: "CLOTHING", items: ["All Clothing","Blazers","Coats & Jackets","Jeans","Knitwear","Blouses","Tops","Cardigans"] },
      { title: "", items: ["Leather Jackets","Trench Coats","Trousers","Skirts","Shorts","Suits","Matching Sets"] },
  ...
  Dresses: {
    categories: [
      { title: "SHOP BY STYLE", items: ["All Dresses","Midi Dresses","Maxi Dresses","Mini Dresses","Wrap Dresses","Slip Dresses"] },
  ...
  Brands: {
    categories: [
      { title: "LUXURY", items: ["The Row","Bottega Veneta","Celine","Loro Piana","Brunello Cucinelli","Valentino"] },
      { title: "CONTEMPORARY", items: ["Toteme","Jacquemus","A.P.C.","Sandro","Maje","AMI Paris","Isabel Marant"] },
      { title: "SUSTAINABLE", items: ["Stella McCartney","Reformation","Eileen Fisher","Nanushka","Ganni","Staud"] },
    ],
  ...
  Shoes: {
    categories: [
      { title: "SHOP BY TYPE", items: ["All Shoes","Heels","Boots","Trainers","Loafers","Sandals","Mules","Ballet Flats"] },
      { title: "SHOP BY BRAND", items: ["Manolo Blahnik","Jimmy Choo","Bottega Veneta","Toteme","The Row","Jacquemus"] },
      { title: "TRENDING NOW", items: ["Ballet Flats","Western Boots","Kitten Heels","Chunky Trainers"] },
    ],
    featured: { title: "Shoe Edit" },
  },
  Bags: {
    categories: [
      { title: "SHOP BY TYPE", items: ["All Bags","Tote Bags","Shoulder Bags","Crossbody Bags","Clutches","Mini Bags"] },
      { title: "SHOP BY BRAND", items: ["Bottega Veneta","Jacquemus","Loewe","The Row","Celine","Polene","Staud"] },
  ...
  Accessories: {
    categories: [
      ...
      { title: "ACCESSORIES", items: ["Sunglasses","Scarves","Belts","Hats","Hair Accessories","Wallets"] },
      { title: "BRANDS", items: ["Bottega Veneta","The Row","Loewe","Sophie Buhai","Completedworks"] },
    ],
  ...
  Beauty: {
    categories: [
      ...
      { title: "MAKEUP", items: ["Foundation","Blush","Lip","Eye","Bronzer","Primers"] },
      { title: "FRAGRANCE", items: ["Perfume","Eau de Toilette","Body Mist","Candles","Discovery Sets"] },
    ],
...
const editorialStories = [
  ...
  { category: "FASHION",   title: "How To Master Off-Duty Style Like Zoe, Kendall & Co.", time: "16 APR 26" },
...
export default function Header({ setPage }) {
  ...
  const pageMap = { "What's New":'plp',Summer:'plp',Brands:'brands',Clothing:'plp',Dresses:'plp',Active:'plp',Shoes:'plp',Bags:'plp',Accessories:'plp',Beauty:'plp',Editorial:'blog' }
  ...
  return (
    <header className="header" onMouseLeave={() => setActiveMenu(null)}>
      <div className="utility-bar">
        ...
        <button className="logo" onClick={() => { setPage('home'); setActiveMenu(null) }}>viva voce</button>
        <div className="utility-right">
          ...
          <button className="icon-btn" onClick={() => setPage('account')} title="Wishlist"><span className="material-icons">favorite_border</span></button>
          <div className="cart-wrap">
            <button className="icon-btn" onClick={() => setPage('cart')} title="Shopping Bag"><span className="material-icons">shopping_bag</span></button>
      ...
      <nav className="main-nav">
        {navLinks.map(label => (
          <button key={label} className={`nav-link ${activeMenu === label ? 'active' : ''}`} onMouseEnter={() => setActiveMenu(label)} onClick={() => handleNav(label)}>
            {label.toUpperCase()}
          </button>
        ))}
      </nav>
